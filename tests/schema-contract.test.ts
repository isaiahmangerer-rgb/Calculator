...
const guestSql = readFileSync(resolve("supabase/migrations/202609100001_guest_public_chat.sql"), "utf8");
describe("database security contract", () => {
  ...
  it("allows guest rooms without exposing private account features", () => {
    expect(guestSql).toContain("auth.jwt()->>'is_anonymous'");
    expect(guestSql).toContain("permanent users access direct messages");
    expect(guestSql).toContain("permanent users manage friend requests");
    expect(guestSql).toContain("A permanent account is required");
  });
});
