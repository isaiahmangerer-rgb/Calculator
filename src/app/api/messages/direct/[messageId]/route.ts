...
export async function PATCH(request: Request, { params }: { params: Promise<{ messageId: string }> }) {
  const viewer = await getPermanentViewer(); if (!viewer) return NextResponse.json({ error: "A permanent account is required." }, { status: 401 }); const { messageId } = await params;
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ messageId: string }> }) {
  const viewer = await getPermanentViewer(); if (!viewer) return NextResponse.json({ error: "A permanent account is required." }, { status: 401 }); const { messageId } = await params; if (!uuidSchema.safeParse(messageId).success) return NextResponse.json({ error: "Invalid message." }, { status: 400 });
