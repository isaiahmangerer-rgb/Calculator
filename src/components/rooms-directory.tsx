...
import { getOrCreateChatIdentity } from "@/lib/guest-chat";
...
export function RoomsDirectory() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        await getOrCreateChatIdentity(supabase);
        const { data, error: roomError } = await supabase.rpc("get_room_summaries");
        if (roomError) throw roomError;
        if (active) setRooms((data as Room[]) ?? []);
      } catch (reason) {
        if (active) setError(reason instanceof Error ? reason.message : "Rooms are unavailable.");
      } finally {
  ...
  if (loading) return <div className="state-card"><LoaderCircle className="spin" /> Joining public chat…</div>;
  ...
  if (error) return <div className="state-card error-state"><Hash /><strong>Guest chat needs one setting.</strong><p>{error}</p><a className="primary-button" href="/sign-in">Sign in instead</a></div>;
