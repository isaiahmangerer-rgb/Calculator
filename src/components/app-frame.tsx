...
type Viewer = { username: string; display_name: string; avatar_url: string | null; isGuest: boolean } | null;
...
export function AppFrame({ children, title, aside }: { children: ReactNode; title: string; aside?: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function connect(user: User | null) {
      ...
      if (!active || !user) { setViewer(null); setOnline([]); return; }
      const { data: profile } = await client.from("profiles").select("username,display_name,avatar_url,status").eq("id", user.id).maybeSingle();
      if (!active || !profile) return;
      setViewer({ ...profile, isGuest: Boolean(user.is_anonymous) });
      channel = client.channel("nexus-presence", { config: { presence: { key: user.id } } });
      channel.on("presence", { event: "sync" }, () => {
        ...
        if (status === "SUBSCRIBED") await channel!.track({ user_id: user.id, username: profile.username, display_name: profile.display_name, avatar_url: profile.avatar_url, status: "online" });
      });
    }

    void client.auth.getUser().then(({ data }) => connect(data.user));
    const { data: authListener } = client.auth.onAuthStateChange((_event, session) => {
      setTimeout(() => { void connect(session?.user ?? null); }, 0);
    });
  ...
  const accountLink = viewer?.isGuest ? "/sign-up" : "/sign-in";
  return (
    <main className="app-canvas inner-app">
      <aside className="left-rail" aria-label="Primary navigation">
        ...
        <div className="rail-account">{viewer ? <><Avatar name={viewer.display_name} image={viewer.avatar_url} /><div><strong>{viewer.display_name}</strong>{viewer.isGuest ? <a href="/sign-up">Temporary guest · create account</a> : <span>@{viewer.username}</span>}</div><button onClick={signOut} aria-label={viewer.isGuest ? "End guest session" : "Sign out"}><LogOut size={17} /></button></> : <><div className="avatar avatar-me">N</div><div><strong>Guest</strong><a href="/rooms">Join chat instantly</a></div></>}</div>
      </aside>
      <section className="main-stage">
        <header className="topbar"><div className="page-title"><span className="brand-mark mobile-brand"><span /></span><strong>{title}</strong></div><div className="top-actions"><a className="icon-button" href="/messages" aria-label="Notifications"><Bell size={18} /></a>{(!viewer || viewer.isGuest) && <a className="primary-button" href={accountLink}>{viewer?.isGuest ? "Create account" : "Sign in"}</a>}</div></header>
