...
export function MessagesDirectory() {
  const supabase = useMemo(() => createClient(), []); const [items, setItems] = useState<Summary[]>([]); const [filter, setFilter] = useState(""); const [guest, setGuest] = useState(false);
  useEffect(() => { if (!supabase) return; void supabase.auth.getUser().then(({ data: auth }) => { if (auth.user?.is_anonymous) { setGuest(true); return; } void supabase.rpc("get_direct_conversation_summaries").then(({ data }) => setItems((data as Summary[]) ?? [])); }); }, [supabase]);
  ...
  if (guest) return <div className="state-card"><MessageCircle /><strong>Create an account for private messages.</strong><p>Guest sessions can join every public room without signing in. DMs remain participant-only.</p><a className="primary-button" href="/sign-up">Create account</a></div>;
